...
export function validateCartItem(request, _response, next) {
  ...
  if (!Number.isSafeInteger(quantity) || quantity < 1 || quantity > 999) errors.push('quantity must be an integer from 1 to 999')
...
export function validateQuantity(request, _response, next) {
  if (!Number.isSafeInteger(request.body?.quantity) || request.body.quantity < 1 || request.body.quantity > 999) {
